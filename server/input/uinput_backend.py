...
try:
    ...
    log.warning(
        "evdev not installed — uinput injection unavailable. "
...
KEY_CODES = (
    {
        ...
        " ": e.KEY_SPACE,
        "ArrowUp": e.KEY_UP,
        "ArrowDown": e.KEY_DOWN,
        "ArrowLeft": e.KEY_LEFT,
        "ArrowRight": e.KEY_RIGHT,
        "Delete": e.KEY_DELETE,
        "Home": e.KEY_HOME,
        "End": e.KEY_END,
        "PageUp": e.KEY_PAGEUP,
...
def _get_uinput():
    ...
    if _ui is None and _evdev_available:
        cap = {
            e.EV_KEY: list(KEY_CODES.values())
        ...
        try:
            ...
            log.error(
                f"Failed to open /dev/uinput: {ex}. "
                "Try: sudo chmod 0666 /dev/uinput"
            )
    return _ui
